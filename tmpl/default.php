...
?>
<div class="mod_wow_raid_progress_classic">
    <?php foreach ($raids as $zoneId => $zone) : ?>
        <ul class="z<?php echo $zoneId; ?>">
            <li class="header">
                <span class="p" style="width:<?php echo $zone['stats']['percent']; ?>%;"></span>
                <?php //echo JHtml::_('link', $zone['link'], JText::_('MOD_WOW_RAID_PROGRESS_CLASSIC_ZONE_' . $zoneId), array('target' => '_blank')); ?>
                <?php echo JText::_('MOD_WOW_RAID_PROGRESS_CLASSIC_ZONE_' . $zoneId); ?>
                <span class="k" title="<?php echo $zone['stats']['percent']; ?>%"><?php echo JText::sprintf('MOD_WOW_RAID_PROGRESS_CLASSIC_MODE_' . strtoupper($zone['stats']['mode']), $zone['stats']['kills'], $zone['stats']['bosses']); ?></span>
            </li>
            <li class="npcs<?php echo ($zone['opened'] == true) ? ' open' : ''; ?>">
                <ul>
                    <?php foreach ($zone['npcs'] as $npc => $data) : ?>
                        <li class="npc">
                            <?php echo JHtml::_('link', $data['link'], JText::_('MOD_WOW_RAID_PROGRESS_CLASSIC_NPC_' . $npc), array('target' => '_blank')); ?>
